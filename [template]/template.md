...
【思路】先考虑单次查询的DP方程，定义$f_{i,0}/f_{i,1}$表示选择$/$不选择$i$的条件下子树的最大点权独立集，则$f_{i,0}=\sum_{j \in son(i)} \max(f_{j,0},f_{j,1})$，$f_{i,1}=\sum_{j \in son(i)}f_{j,0}$。一条链时同上题序列上动态DP的思路，树上可以考虑树链剖分，将转移方程中来自虚儿子的项记作$g$，则$g_{i,0}=\sum_{j \neq heavy\_son(i)}\max(f_{j,0},f_{j,1})$，$g_{i,1}=\sum_{j \neq heavy\_son(i)}f_{j,0}$。将转移方程写成广义矩阵乘法形式，有$\begin{bmatrix}f_{i,0}\\f_{i,1}\end{bmatrix}=\begin{bmatrix}g_{i,0}&g_{i,0}\\g_{i,1}&-\infty\end{bmatrix}\begin{bmatrix}f_{heavy\_son(i),0}\\f_{heavy\_son(i),1}\end{bmatrix}$。用LCT维护轻重链的信息，$f$是重链的总答案，$g$是来自虚儿子的答案。
...
{
    int fa, son[2];
    Matrix f, g; // f-当前节点以下的链的答案 g-维护虚儿子的信息
} tree[MAXN]; // LCT维护子树信息模板
...
inline void rotate(int u)
{
    ...
    tree[u].fa = ffa;
    if (!isroot(fa)) tree[ffa].son[dffa] = u;
...
int main()
{
    ios::sync_with_stdio(false); cin.tie(nullptr); // 关闭与stdio的同步，加速输入
    int n, m; cin >> n >> m; // n-节点数 m-询问数
...
【题目】一个$n$行$m$列的矩阵，权值为$0$的位置是关键点，求选择权值非零的点使所有关键点连通的最小代价，并输出选择的方案。

【思路】**斯坦纳树：带权无向图中连通所有给定点的最小权值生成树。**设$f(i,j,S)$表示点$(i,j)$连通位标识$S$指示的关键点的最小代价，$w(i,j)$表示点$(i,j)$的权值。合并连通分量的转移方程：$f(i,j,S)=f(i,j,k)+f(i,j,S\ xor\ k)-w(i,j)\ for\ k \subset S$，更新周围的转移方程：$f(i,j,S)=\min\{f(i\pm1,j\pm1,S)+w(i,j)\}$。第二个转移方程应该按照SPFA的顺序转移。

```c++
const int dir[][2] = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
int w[15][15], dp[15][15][1 << 12]; // w-权值
tuple<int, int, int> pre[15][15][1 << 12]; // pre-状态转移的前驱 #include<tuple>
...
int main()
{
    ...
    rep(i, 1, n) puts(plan[i] + 1); // 输出方案 'x'-关键点 'o'-选择的点 '_'-未选择的点
    return 0;
}
```
...
【思路】二分图左部一个节点代表一行，右部一个节点代表一列，网格中的节点表达为对应行列节点的连边，问题转换为用最少的点覆盖所有的边。匈牙利算法求最大匹配，最大匹配等价于最小点覆盖。
...
bool augment(int u) // DFS増广 u-DFS树的根节点
{
    ...
    for (int i = head[u]; ~i; i = edges[i].nxt)
    {
        ...
        {
            match[u] = v, match[v] = u; // 反转増广路上的匹配，使匹配数+1
            return true;
        }
    }
    return false;
...
void mincover(int nx, int ny) // nx-左支节点数量 ny-右支节点数量
{
    ...
    for (int i = 1; i <= nx; i++)
        if (!match[i])  // 左部没有匹配的点一定连接没有覆盖的边
            augment(i); // 最小点覆盖应该选择与它连通的所有匹配点对的右部
...
{
    int r, c, n;
    while (readi(r, c, n) != EOF && (r | c | n)) // r-行数 c-列数 n-敌人数
    {
        clr(head, -1), edgeid = 0 // 链式前向星初始化
        repne(i, 0, n)
        {
            int x, y; readi(x, y);
            addedge(x, r + y); // 离散建图，二分图右部从r+1开始编号
        }
        mincover(r, c);
    }
    return 0;
...
【题目】有源汇有上下界最大流模板题，网络的所有边有最小流量限制，求源点$src$到汇点$dst$的最大流量。

【思路】

* 建图方法：添加虚拟源点$isrc$、汇点$idst$，对于有下界的边$<from,to,lower,upper>$，分离成边$<from,idst,lower>,\ <isrc,to,lower>,\ <from,to,upper-lower>$三条只有上界的边。

* 判断是否有解：分离出下界建图后，连边$e=<dst,src,+\infty>$，保证源汇点流量平衡。求$isrc$到$idst$的最大流，记流过$e$的流量为$f_1$，如果最大流等于所有下界流量之和，则存在可行解。
* 求最大/最小流：删除虚拟源汇点和边$e$以及关联的边。如果求最大流，在残量网络上求$src$到$dst$的最大流，记为$f_2$，最终答案为$f_1+f_2$。如果求最小流，则在残量网络上求$dst$到$src$的最大流，记为$f_3$，最终答案为$f_1-f_3$。
...
void addflow(int from, int to, int flow) // 连只有上界的边
{
    if (flow == 0) return;
    addedge(from, to, flow);
    addedge(to, from, 0);
}
void addflow(int from, int to, int lower, int upper) // 连有上下界的边
{
...
int dep[MAXN], cur[MAXN]; // Dinic算法模板
bool bfs(int n, int src, int dst)
{
    fill_n(dep, n + 1, 0), dep[src] = 1;
...
int main()
{
    ...
    repne(i, 0, m)
    {
        int u, v, lower, upper; readi(u, v, lower, upper);
    ...
    if (dinic(n + 2, isrc, idst) == sum) // 如果虚拟源汇的流量能流满，则有可行解
    {
        ...
        for (int i = head[isrc]; ~i; i = edges[i].nxt)
            edges[i ^ 1].to = 0; // 注意在Dinic模板中跳过被删除的边，即to==0的边
...
【题目】最小费用最大流模板题，每条边按照流量计费，求源点到汇点的最大流和达到最大流的最小费用。
...
bool spfa(int n, int src, int dst)
{
    ...
    while (!Q.empty())
    {
        ...
        for (int i = head[u]; ~i; i = edges[i].nxt)
        {
            ...
            dis[e.to] = dis[u] + e.dis, pre[e.to] = i;
            if (!vis[e.to])
            {
                Q.push(e.to);
                vis[e.to] = true;
