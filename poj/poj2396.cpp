...
bool getdep()
{
    clr(dep,0); dep[vsrc]=1;
    queue<int> Q; Q.push(vsrc);
    while(!Q.empty())
    {
        int u=Q.front(); Q.pop();
...
int dinic()
{
    ...
    while(getdep())
    {
        ...
        while(f=augment(vsrc,INF))maxflow+=f;
    }
    return maxflow;
}
...
int main()
{
    ...
    while(T--)
    {
        ...
        int c; readi(c);
        clr(low,0); clr(high,INF);
        rep(i,1,c)
        {
            int x,y; char con[4]; int z;
